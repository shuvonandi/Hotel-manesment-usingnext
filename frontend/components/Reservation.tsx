...
const Reservation = ({
  ...
  const formateDateForStrapi = (date: Date) => {
  ...
  const saveReservation = ()=>{
    ...
    //filter reservation for the current room and check if any reservation overlaps eith the selected dates
    const isReserved =reservations.data.filter((item: any)=> item.attributes.room.data.id===room.data.id          //filter reservation for the current room
      ).some((item:any)=>{
    ...
    const data = {
      data: {
        ...
        checkIn: checkInDate ? formateDateForStrapi(checkInDate) : null, //format selected check-in date
        checkOut: checkOutDate ? formateDateForStrapi(checkOutDate) : null, //format selected check-out date
        room: room.data.id,
      },
    };
    ...
    // refress the page to reflect the updates reservation status
    router.refresh();
    }
  };
    ...
    <div>
      <div className="bg-tertiary h-[320px] mb-4 ">
        ...
        <div className="bg-accent py-4 text-center relative mb-2">
          ...
          {/* triangle */}
          <div className="absolute -bottom-[8px] left-[calc(50%_-_10px)] w-0 h-0 border-l-[10px] border-l-transparent border-t-[8px] border-accent border-r-[10px] border-r-transparent">
